...
with st.sidebar:
    ...
    if st.button("👥 Evaluate Team", use_container_width=True, type="primary" if st.session_state.mgr_section == "Evaluate Team" else "secondary"):
        st.session_state.mgr_section = "Evaluate Team"; st.rerun()
    if st.button("✍️ Self–Evaluation", use_container_width=True, type="primary" if st.session_state.mgr_section == "Self-Evaluation" else "secondary"):
        st.session_state.mgr_section = "Self-Evaluation"; st.rerun()
    if st.button("📜 View History", use_container_width=True, type="primary" if st.session_state.mgr_section == "History" else "secondary"):
        st.session_state.mgr_section = "History"
...
if st.session_state.mgr_section == "Evaluate Team":
    ...
    with st.expander("Open Self-Evaluation Form", expanded=True):
        ...
        if set(all_criteria).issubset(submitted_criteria):
            ...
            for crit, weight in development_criteria:
                if crit not in submitted_criteria:
                    all_scores[crit] = st.number_input(
                        ...
                        value=0,
                        step=1,
                        key=f"{name}_{crit}_dev_self_{selected_quarter}"
                    )
            ...
            for crit, weight in other_aspects_criteria:
                if crit not in submitted_criteria:
                    all_scores[crit] = st.number_input(
                        ...
                        value=0,
                        step=1,
                        key=f"{name}_{crit}_other_self_{selected_quarter}"
                    )
            ...
            for crit, weight in foundational_criteria:
                if crit not in submitted_criteria:
                    all_scores[crit] = st.number_input(
                        ...
                        value=0,
                        step=1,
                        key=f"{name}_{crit}_found_self_{selected_quarter}"
                    )
            ...
            for crit, weight in futuristic_criteria:
                if crit not in submitted_criteria:
                    all_scores[crit] = st.number_input(
                        ...
                        value=0,
                        step=1,
                        key=f"{name}_{crit}_fut_self_{selected_quarter}"
                    )
            ...
            if all_scores and st.button(f"Submit Your Self-Rating for Quarter {selected_quarter}"):
                current_year = datetime.datetime.now().year # Get the current year for the submission
